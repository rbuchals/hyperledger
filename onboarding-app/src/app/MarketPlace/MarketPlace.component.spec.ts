...
describe('MarketPlaceComponent', () => {
  ...
  it('should update the table when a MarketPlace is added', fakeAsync(() => {
    ...
    sinon.stub(component.serviceMarketPlace, 'addAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a MarketPlace is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceMarketPlace, 'updateAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
    ...
    let mockForm = new FormGroup({
      marketPlaceId: new FormControl('id')
    });
  ...
  it('should update the table when a MarketPlace is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceMarketPlace, 'deleteAsset').returns(new Observable<any>(observer => {
      observer.next('');
      observer.complete();
    }));
